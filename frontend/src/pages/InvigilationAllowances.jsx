...
function InvigilatorsTable({ rows, faculty }) {
  ...
  const printReport = () => {
    ...
    win.document.write(`<!DOCTYPE html><html><head><title>Exam Invigilators — ${facultyLabel}</title>
      ...
      <h2 style="margin:0;color:#1a3a5c;">CABE Exam Operations — Exam Invigilators</h2>
      <p style="color:#666;margin:4px 0;">${facultyLabel} | Mid-Semester 2025/2026 | 6th–10th July 2026</p>
      <p style="font-size:11px;color:#888;">Senior Member: GHS 60/session · Other staff: GHS 30/session · 10% deduction applied</p>
  ...
  const exportExcel = () => {
    ...
    a.href = url; a.download = `Invigilators_${facultyLabel.replace(/\s+/g,'_')}.xls`; a.click();
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <span className="inline-block px-3 py-1 rounded-full text-sm font-semibold bg-blue-100 text-blue-800">Exam Invigilators</span>
          <span className="text-sm text-gray-500">{filtered.length} staff · Sr. Member GHS 60/session · Others GHS 30/session · 10% deduction</span>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={printReport} className="bg-gray-800 hover:bg-gray-900 text-white text-xs px-3 py-1.5 rounded-lg font-semibold flex items-center gap-1.5">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.length === 0 && <tr><td colSpan={12} className="px-4 py-6 text-center text-gray-400 text-sm">No staff in this faculty</td></tr>}
              {filtered.map((r, i) => (
...
function OfficeStaffTable({ rows, faculty }) {
  ...
  const exportExcel = () => {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `OfficeStaff_${facultyLabel.replace(/\s+/g,'_')}.xls`; a.click();
    URL.revokeObjectURL(url);
  };
